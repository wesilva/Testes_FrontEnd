...
describe('ProductList', () => {
  ...
  it('should display error message when promise rejects', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryAllByTestId('product-cart')).toHaveLength(0);
    });
  });

  it('should filter the product list when a search is performed', async () => {
    const searchTerm = 'Rolex';
    ...
    await waitFor(() => {
      expect(screen.getAllByTestId('product-cart')).toHaveLength(3);
    });
    ...
    await waitFor(() => {
      expect(screen.getAllByTestId('product-cart')).toHaveLength(1);
    });
  });
  it('should display the total quantity of produts', async () => {
    server.createList('product', 10);
